...
dfs = ["matches_with_strengths", "matches_with_strengths_weighted", "matches_with_strengths_weighted_opponents", "matches_with_strengths_weighted_form"]
...
for name in dfs:
    ...
    for _,match in df.iterrows():
        ...
        for h in range(MAX_GOALS):
            for a in range(MAX_GOALS):
                ...
                # dixon-coles adjusment
                if h == 0 and a == 0:
                    adjustment = 1 - (home_xg * away_xg * RHO)
                elif h == 0 and a == 1:
                    adjustment = 1 + (home_xg * RHO)
                elif h == 1 and a == 0:
                    adjustment = (away_xg * RHO)
                elif h == 1 and a == 1:
                    adjustment = 1 - RHO
                else:
                    adjustment = 1
    ...
    new_df.to_csv(f'../Data/DC_{name}_predictions.csv')
    print("Saved dataset")