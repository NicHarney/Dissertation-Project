...
for season in target_seasons:
    if season == '2017-18':
        all_stats = stats[stats['Div'] == season] # Filter stats to include only seasons before the target season
    else:
        all_stats = stats[stats['Div'] < season] # Filter stats to include only seasons before the target season
    ...
    for team in team_listing:
        team_data = all_stats[all_stats['Team'] == team].sort_values(by='Div', ascending=True) # Apply weightings to each stat (example: 0.5 for most recent season, 0.3 for second most recent, 0.2 for third) weights = [0.5, 0.3, 0.2] weighted_stats = team_data[stat_columns].multiply(weights[:len(team_data)], axis=0).sum() weighted_stats['Team'] = team team_weightings = team_weightings.append(weighted_stats, ignore_index=True) team_weightings.to_csv('../data/processed/team_weighted_profiles.csv', index=False)

        seasons_count = len(team_data)

        weights = np.arange(1, seasons_count + 1)
        weights = weights / weights.sum() # Normalize weights to sum to 1 weighted_stats = team_data[stat_columns].multiply(weights[:seasons_count], axis=0).sum() weighted_stats['Team'] = team team_weightings = team_weightings.append(weighted_stats, ignore_index=True) team_weightings.to_csv('../data/processed/team_weighted_profiles.csv', index=False)
